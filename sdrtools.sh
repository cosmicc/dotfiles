...
echo -n "${YEL}Download, Compile & Install APRX (y/n)? ${NC}" 
read answer
if [ $answer = "y" ]; then
    sudo apt -qq install aprx -y
fi

echo -n "${YEL}Download, Compile & Install APRSDigi (y/n)? ${NC}" 
read answer
if [ $answer = "y" ]; then
    mkdir /opt/build/radio-tools
    echo "${CYN}Installing APRSDigi...${NC}"
    if [ ! -d "/opt/build/radio-tools/aprsdigi" ]; then
        echo "${CYN}Cloning APRSDigi Source...${NC}"
        git clone https://github.com/n2ygk/aprsdigi.git
        cd aprsdigi
    else
        echo "${CYN}Updating APRSDigi Source...${NC}"
        cd aprsdigi
        git pull
    fi
    echo "${CYN}Compiling & Installing APRSDigi...${NC}"
